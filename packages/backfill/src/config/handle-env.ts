...
let initialEnv: Backfill.Utility.ProcessEnv | undefined = undefined;
let combinedEnv: Backfill.Utility.ProcessEnv | undefined = undefined;
let cachedLoadedEnvFiles: Backfill.Utility.LoadedEnvFiles = [];
let previousLoadedEnvFiles: Backfill.Utility.LoadedEnvFiles = [];

export function processEnv(
  loadedEnvFiles: Backfill.Utility.LoadedEnvFiles,
  dir?: string,
  log: Backfill.Utility.Log = console,
  forceReload = false
) {
  if (!initialEnv) {
  ...
  if (
    ...
    return process.env as Backfill.Utility.ProcessEnv;
  }
...
export function loadEnvConfig(
  ...
  combinedEnv: Backfill.Utility.ProcessEnv;
  loadedEnvFiles: Backfill.Utility.LoadedEnvFiles;
} {
  if (!initialEnv) {
    initialEnv = Object.assign({}, process.env);
